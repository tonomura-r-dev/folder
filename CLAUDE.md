...
- 2026-09-01: 案件引き継ぎ用スプレッドシートの構成案を作った（`_drafts/案件引き継ぎシート_構成案.md`）。
  - Web広告＋LINEOA統合版。①INDEX（案件マスタ）／②Web広告_引き継ぎ／③LINEOA_引き継ぎ／
    ④入力ルール・凡例 の4シート構成、案件IDで紐付け。
  - **実物の現行スプレッドシートは未閲覧のまま作った。** クラウド環境からは
    `docs.google.com` が egress ブロックで開けない（Sheets連携も未接続）。依頼文中の
    「現状の主な項目（参考）」欄と3要件だけを根拠にゼロから設計している。
  - 実物の列見出しが手に入ったら、この構成案とのズレを突き合わせて微調整すること。

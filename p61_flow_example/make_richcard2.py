...
def card_frame(d, top, height, brand):
    d.text((20, top-20), f"{brand}●LINE公式アカウント", font=font(10), fill=LINE_GREEN)
    box = (20, top, W-20, top+height)
    d.rounded_rectangle(box, radius=4, fill="#FFFFFF")
    d.polygon([(box[0], top+16), (box[0]-8, top+21), (box[0], top+26)], fill="#FFFFFF")
    return box
...
def panelA():
    ...
    y = chat_top + 60
    card_h = 372
    box = card_frame(d, y, card_h, "ハコブーン")
    yy = y + 16
    ...
    d.text((box[0], box[3]+8), "18:02", font=font(9), fill="#8A93A8")

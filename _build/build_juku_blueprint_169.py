...
header(s, "施策の設計図①　友だち追加の動線",
       "どこで、何を訴求して、月に何人がLINEの友だちになるか（中堅チェーンの本部アカウント1つのモデル値）",
       [1, 2, 4], "※友だち追加数は中堅チェーンの本部アカウント1つのモデル値（例）。チラシ・教室のQRは計測できないため数えていない。費用は税抜")
...
header(s, "施策の設計図②　効率改善（ステップ配信・企画配信）",
       ...
       [3, 5, 6], "※本部アカウント1つのモデル値。開封率・クリック率＝DYM SIMの係数（ステップ 72.5%×12%／企画 78%×10%）、CVR＝0.3〜1.0%。件数は四捨五入。通知メッセージのみ別途費用")
HW = (W - 0.8) / 2
...
table(X0 + HW + 0.8, "[企画配信]　時期に合わせて月1〜2本",
      [("1〜2月", "新学年で変わること（学年別）＋体験のご案内", "5件", "CV② 体験予約", "6,800人×78%×10%×1.0%", True),
       ...
       ("テスト後", "点数の見方＋苦手単元の無料チェック（中学生の保護者）", "2件", "CV② 体験予約", "2,700人×78%×10%×0.8%", True),
       ("講習の前", "体験・入会に至らなかった人へ再案内（通知メッセージ）", "実績で計算", "⑥再育成", "別途費用", False)],
      "対象：友だち 累計約10,000人（ブロックを除いて約6,800人）")
box(s, X0, 15.95, W, 1.9, BEIGE, [("ステップ配信で毎月　資料請求 約5件・体験予約 約4件", 18, True, INK),
                                   ("企画配信は1回あたり　体験予約 2〜5件", 15, False, INK)], adj=0.12)

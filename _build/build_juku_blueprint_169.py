...
header(s, "施策の設計図①　友だち追加の動線",
       "どこで、何を訴求して、月に何人がLINEの友だちになるか（1教室あたりのモデル値）",
       [1, 2, 4], "※友だち追加数は1教室あたりのモデル値（例）。費用は税抜。離脱防止ポップアップはSitelead等のツールを使う")
COLS = [("箇所", 8.0), ("訴求", 12.6), ("LINE追加", 5.2), ("費用", 5.67)]
x = X0
for t, w_ in COLS:
    box(s, x, 4.3, w_ - 0.2, 0.8, "EEEEEE", [(t, 13, True, INK)], shape=MSO_SHAPE.RECTANGLE)
    x += w_
ROUTES = [("00", "離脱防止ポップアップ", "「まだ決めなくて大丈夫です」", "200人", "月3万円\n初期1.5万円"),
          ("01", "完了画面からの誘導", "「日程の確認・変更はLINEで」", "30人", "月5万円\n初期10万円"),
          ("02", "チラシ・教室のQR", "「LINE限定 テスト対策プリント」", "20人", "費用内")]
for i, (no, where, msg, n, fee) in enumerate(ROUTES):
    y = 5.35 + i * 2.75
    h = 2.45
    box(s, X0, y, 7.8, h, "FFFFFF", [], line="8EA9DB", shape=MSO_SHAPE.RECTANGLE)
    box(s, X0 + 0.3, y + 0.6, 1.4, 1.25, NAVY, [(no, 16, True, "FFFFFF")], shape=MSO_SHAPE.OVAL, ml=0.02)
    ...
    box(s, X0 + 20.6 + 0.65, y, 4.35, h, LINE_GREEN, [(n, 26, True, "FFFFFF"), ("／月", 13, True, "FFFFFF")], shape=MSO_SHAPE.RECTANGLE)
    box(s, X0 + 25.8, y, 5.47, h, "FFFFFF", [(ln, 14, False, INK) for ln in fee.split("\n")], line="C9D3E6", shape=MSO_SHAPE.RECTANGLE)
box(s, X0, 13.85, W, 1.6, LINE_GREEN, [("新しい友だち　合計 月250人（1教室あたり）", 22, True, "FFFFFF")], adj=0.15)
text(s, X0, 15.85, W, 1.6, [("一番大きいのは、サイトから帰ろうとする保護者を拾う「離脱防止ポップアップ」。", 15, True, NAVY),
...
header(s, "施策の設計図②　効率改善（ステップ配信・企画配信）",
       ...
       [3, 5, 6], "※1教室あたりのモデル値。開封率・クリック率＝DYM SIMの係数（ステップ 72.5%×12%／企画 78%×10%）、CVR＝0.3〜1.0%。費用は月額費用内（講習時期の再案内のみ別途）")
HW = (W - 0.8) / 2
...
table(X0, "[ステップ配信]　友だち追加から14日間・自動",
      [("0日後", "あいさつ＋30秒診断 → 資料を受け取る", "0.3件", "CV① 資料請求", "250人×100%×12%×1.0%", True),
       ("3日後", "家でできる勉強のコツ", "信頼づくり", "CVは狙わない", "250人×72.5%×12%", False),
       ("5日後", "月謝＋講習費の年間の目安", "0.2件", "CV① 資料請求", "250人×72.5%×12%×1.0%", True),
       ("7日後", "同じタイプの子の事例＋体験のご案内", "0.2件", "CV② 体験予約", "250人×72.5%×12%×0.8%", True),
       ("14日後", "次のテストから逆算＋体験のご案内", "0.2件", "CV② 体験予約", "250人×72.5%×12%×1.0%", True)],
      "対象：新しい友だち 月250人")
table(X0 + HW + 0.8, "[企画配信]　月1〜2本",
      [("講習の前", "講習・新学年の先行案内", "0.5件", "CV② 体験・講習", "680人×78%×10%×1.0%", True),
       ("テストの前", "テスト対策講座のご案内（中学生の保護者）", "0.1件", "CV② 体験予約", "270人×78%×10%×0.5%", True),
       ("資料請求後", "事例・費用のQ&A・教室の紹介", "実績で計算", "CV② 体験予約", "資料を請求した人", False),
       ("講習の時期", "体験・入会に至らなかった人へ再案内", "実績で計算", "通知メッセージ", "別途費用", False)],
      "対象：友だち 累計約1,000人（ブロックを除いて約680人）")
box(s, X0, 15.95, W, 1.9, BEIGE, [("1教室あたり月に　資料請求 約0.5件・体験予約 約1.0件", 18, True, INK),
                                   ("100教室なら　資料請求 約50件・体験予約 約100件／月", 14, False, INK)], adj=0.12)

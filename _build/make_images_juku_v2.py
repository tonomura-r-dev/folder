...
# v2（2026-09-25）：仕込み開始線（26日前）を削除した版。出力は juku_v2_trend_*.png
...
# （v2）仕込み開始線は削除
ax.set_title("「塾」の検索推移｜ピークは2月上旬",
             fontsize=12, fontweight="bold", color=NAVY, pad=26, loc="left")
fig.tight_layout()
fig.savefig(OUTDIR / "juku_v2_trend_juku.png", bbox_inches="tight", facecolor="white")
plt.close(fig)
...
fig.tight_layout()
fig.savefig(OUTDIR / "juku_v2_trend_keitai.png", bbox_inches="tight", facecolor="white")
plt.close(fig)
...
ax.set_title("季節講習の検索推移｜夏が圧倒的（冬期・春期の3〜5倍）",
             fontsize=12, fontweight="bold", color=NAVY, pad=26, loc="left")
fig.tight_layout()
fig.savefig(OUTDIR / "juku_v2_trend_koushuu.png", bbox_inches="tight", facecolor="white")
plt.close(fig)
...
fig.tight_layout()
fig.savefig(OUTDIR / "juku_v2_trend_juken.png", bbox_inches="tight", facecolor="white")
plt.close(fig)

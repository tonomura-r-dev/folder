...
set_para_text(lead, ["塾を調べているのは30代後半〜50代半ばの母親（中心は40代）。一番困っているのは、料金ではなく",
...
GRID = [["🎯 30秒ぴったり診断", "💰 コース・料金", "🏫 教室・講師紹介"],
        ["📄 資料を受け取る（CV①）", "💬 よくある質問", "📅 無料体験・教室見学の予約（CV②）"]]
...
box(s, 1.2, 13.5, 12.3, 2.3, PALE, [("体験予約した人", 11, True, GREEN), ("→ 前日リマインドへ（P21）", 10.5, False, INK)], line="8EA9DB")
...
s = rebuild(S(21), "実文面：場面ごとのメッセージ",
            ...
            "※事例の数字は貴社の実績に差し替え、学年・期間・通った回数を書く。体験予約の前日のメッセージはP21", phases=[3, 5])
...
SRCS = [("P3", "厚労省 人口動態統計／経産省 特定サービス産業動態統計／文科省 学校基本調査／矢野経済研究所 2024"),
        ...
        ("P6・P8・P19", "LINEヤフー前後検索 直近1年（取得中）KW：塾／個別指導／体験授業／高校受験"),
        ("P7・P23", "Googleトレンド 日本 2026年年初来（実測）"),
...
# ================================================================ 並べ替え・削除
ORDER = [1, 27, 2, 3, 4, 10, 9, 14, 11, 5, 12, 26, 13, 15, 16, 17, 18, 19, 20, 21, 23, 25, 24, 8, 7, 6, 36, 28, 29, 30, 31, 32, 34, 35]
DROP = [22, 33]   # 友だち紹介の2枚（旧P15・旧P25）は外す（2026-09-25 殿村さん指示）

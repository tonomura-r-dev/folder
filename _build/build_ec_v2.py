...
def phone(slide, x, y, w, h, header, lines):
    ...
    hd = box(slide, x + 0.14, y + 0.42, w - 0.28, 0.52, fill=GREEN,
             shape=MSO_SHAPE.RECTANGLE)
    put_text(hd.text_frame, [one(header, 10, True, WHITE, align="c")],
...
PW, PH = 8.0, 6.2
phone(s, CX0 + 0.3, CY0 + 0.15, PW, PH, "Day0｜初動",
      [("in", "友だち追加ありがとうございます！\n初回10%OFFクーポンをお届けします🎁"),
       ("chip", "クーポンを使う"),
       ("note", "通知：友だち追加ありがとうございます！初回1…")])
phone(s, CX0 + 8.7, CY0 + 0.15, PW, PH, "Day4｜不安つぶし",
      [("in", "「送料はいくら？」というお声を\nよくいただきます。全国一律550円、\n11,000円以上で無料です"),
       ("in", "返品は到着後8日以内、\n未開封なら送料弊社負担です"),
       ("chip", "商品を見てみる")])
phone(s, CX0 + 17.1, CY0 + 0.15, PW, PH, "Day7｜クロージング",
      [("in", "クーポンは本日23:59まで。\nカートに残っている商品があります"),
       ("btn", "クーポンを使って購入する"),
       ("note", "通知：【本日まで】10%OFFクーポンの…")])

YB = CY0 + PH + 0.70
T(s, CX0, YB, CW, 0.80,
  [one("── 3通の役割", 14, True, TNAVY)], anchor="m")
r22 = [
    ("Day0", "取りこぼさない", "追加直後が一番熱い。期限を切って初回の背中を押す"),
    ("Day4", "不安を消す", "カゴ落ち理由1位は送料の不透明さ。先に答えを出す"),
    ("Day7", "刈り取る", "期限リマインドがこの7日間で最大の山になる"),
]
rw2 = (CW - 0.6) / 3
for i, (t1, t2, t3) in enumerate(r22):
    x = CX0 + i * (rw2 + 0.3)
    box(s, x, YB + 0.90, rw2, 2.90, fill=PALE, line=None)
    tb = s.shapes.add_textbox(Cm(x + 0.25), Cm(YB + 1.15), Cm(rw2 - 0.5), Cm(0.70))
    put_text(tb.text_frame,
             [multi([(t1 + "　", 12, True, INK),
                     (t2, 14, True, ORANGE)], align="c")],
             anchor="m")
    T(s, x + 0.25, YB + 1.95, rw2 - 0.5, 1.70,
      [one(t3, 12, None, INK, ls=1.30)], anchor="t")
...
PH_W, PH_H = 8.0, 6.2
phone(s, CX0 + 0.3, CY0 + 0.15, PH_W, PH_H, "① 購入直後",
      [("in", "ご購入ありがとうございます！\n発送予定は2〜3営業日以内です"),
       ("in", "お届けまでの間に、\n使い方のコツをご案内します"),
       ("chip", "使い方ガイドを見る"),
       ("note", "※ここでレビューは求めない")])
phone(s, CX0 + 8.7, CY0 + 0.15, PH_W, PH_H, "② 使い始めの頃",
      [("in", "お使いいただけていますか？"),
       ("in", "こう使うと実感しやすい、\nという声が多いポイントを\nまとめました"),
       ("chip", "ポイントを見る"),
       ("note", "※離脱の兆しをここで拾う")])
phone(s, CX0 + 17.1, CY0 + 0.15, PH_W, PH_H, "③ 無くなる少し前",
      [("in", "そろそろ無くなる頃かと\nお送りしました"),
       ("in", "同じ商品をもう一度お届けするか、\n定期便に切り替えるか選べます"),
       ("btn", "再購入する"),
       ("chip", "定期便に切り替える")])
...
T(s, CX0, YA, CW, 0.80,
  [one("── 3通の役割", 14, True, TNAVY)], anchor="m")
roles = [
    ("① 購入直後", "不安をなくす", "届くまでの間を埋める。ここでレビューを求めると信頼を削る"),
    ("② 使い始めの頃", "離脱を防ぐ", "使い方でつまずく人を拾う。F2の成否はここで決まる"),
    ("③ 無くなる少し前", "F2を取る", "最大の山。タイミングは「内容量÷1日使用量」から逆算する"),
]
rw = (CW - 0.6) / 3
for i, (t1, t2, t3) in enumerate(roles):
    x = CX0 + i * (rw + 0.3)
    box(s, x, YA + 0.90, rw, 2.90, fill=PALE, line=None)
    tb = s.shapes.add_textbox(Cm(x + 0.25), Cm(YA + 1.15), Cm(rw - 0.5), Cm(0.70))
    put_text(tb.text_frame,
             [multi([(t1 + "　", 12, True, INK),
                     (t2, 14, True, ORANGE)], align="c")],
             anchor="m")
    T(s, x + 0.25, YA + 1.95, rw - 0.5, 1.70,
      [one(t3, 12, None, INK, ls=1.30)], anchor="t")

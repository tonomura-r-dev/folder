...
def make_line_notif(filename, accent, brand_letter, brand_name, title, body, caption):
    ...
    # LINE app header - WHITE with black icons (real LINE style)
    header_top = 50
    header_h = 54
    ...
    # LINE bubble tail (small triangle top-left)
    ...
    draw.text((bubble_box[2]+6, bubble_box[3]-16), "18:02\n既読", font=font(9), fill="#AAAAAA")

    # ---- Rich menu (signature LINE OA element) ----
    rm_top = chat_bottom
    draw.rectangle((12, rm_top, W-12, rm_top+richmenu_h), fill="#FAFAFA")
    draw.line((12, rm_top, W-12, rm_top), fill="#E5E5E5", width=1)
    labels = [("🏠", "サイトTOP"), ("🎁", "クーポン"), ("📞", "お問合せ")]
    cell_w = (W-24)/3
    for i,(icon,label) in enumerate(labels):
        cx0 = 12 + i*cell_w
        if i>0:
            draw.line((cx0, rm_top+8, cx0, rm_top+richmenu_h-8), fill="#E5E5E5", width=1)
        bbox = draw.textbbox((0,0), icon, font=font(30))
        iw = bbox[2]-bbox[0]
        draw.text((cx0+cell_w/2-iw/2, rm_top+30), icon, font=font(30), fill=accent)
